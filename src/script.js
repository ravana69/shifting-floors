...
async function Draw(){
  ...
  if(!t){
    ...
    mkP = () => {
      ...
      tall = C(t) * 2
      P = []
      if(typeof flashes == 'undefined') flashes = Array(iPc).fill().map(v=>[0,0])
      Array(iPc).fill().map((v,i)=>{
        a = []
        for(j=4;j--;){
          n = (((j+1)/2)|0)%2?i/cl|0:(i/cl|0)+1
          ls = 2**.5/3
          tx = ((i%cl)-cl/2+.5) / cl * wide
          ty = S(-C(t/2)*12+Math.PI*4/cl*(j/2|0%2?i:i+1)) * tall * S(q=Math.PI*4/rw*n)
          tz = ((i/cl|0)-rw/2+.5) / rw * deep
    ...
    starImgs = Array(10).fill().map((v,i) => {
      ...
      a.img.src = `https://srmcgann.github.io/temp/stars/star${i?i:''}.png`
      return a
    })
    
    bounding = Cylinder(1,20,10,2)
  }
  
  mkP()
  
  oX=0, oY=0, oZ=Math.min(16,Math.max(8, (.3+C(t/2))*40))
  Rl=0, Pt=-.75+C(t/3)/2, Yw = C(t/2)*2
  ...
  P.map((v,i)=>{
    ...
    col1 = `hsla(${360/1/B.length*flashes[i][1]},99%,${100-flashes[i][0]*50}%,${.1+flashes[i][0]*3})`
    col2 = `hsla(${360/1/B.length*flashes[i][1]},99%,${Math.max(50,100-flashes[i][0]*50)}%,${Math.min(.5,.025+flashes[i][0])})`
    ...
    B.map(q=>{
      ...
      Y1 = q[1]
      Z1 = q[2]
      if(Math.hypot(X1-ax,Y1-ay,Z1-az)<ls*2){
        X2 = X1 + q[3]*anticipation
        Y2 = Y1 + .4 + q[4]*anticipation
        Z2 = Z1 + q[5]*anticipation
        if(l=lineFaceI(X1,Y1,Z1,X2,Y2,Z2,v[0])){
          n = l[1]
          d = Math.hypot(...n)
          n[0] /= d
          n[1] /= d
  ...
  B.map((v,i) => {
    ...
    R(Rl,Pt,Yw,1)
    if(Z>0){
      s = Math.min(3e3,3e3/Z*(.33+v[6]/3))
      l = Q()
      x.globalAlpha = (v[6]*3)**2
      x.drawImage(starImgs[1+(i%8)].img,l[0]-s/2.07,l[1]-s/2.07,s,s)
    }
    v[6] -= .005
  })
